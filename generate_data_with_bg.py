...
from scipy.signal import convolve,fftconvolve,stft
import time
...
def save_time_and_freq_plots(a1,a2,voice_idx):
    plt.figure(figsize = (12,10))
    plt.subplot(2,1,1)
    plt.plot(a1)
    plt.title('Original Sound (Normalized)')
    plt.xlabel('Sample Index')
    plt.ylabel('Amplitude')
    
    plt.subplot(2,1,2)
    plt.plot(a2)
    plt.title('Post Rir Sound')
    plt.xlabel('Sample Index')
    plt.ylabel('Amplitude')
    plot_path = os.path.join(os.getcwd(), f'Time_Domain_vs_freq_Domain_Room_generation_{voice_idx}.png')
    plt.tight_layout()
    plt.savefig(plot_path)
    plt.close()
    print("saved")

    _, _, s1 = stft(a1, fs=44100, nperseg=1024)
    _, _, s2 = stft(a2, fs=44100, nperseg=1024)
    s1dB = 20 * np.log10(np.abs(s1) + 1e-8)
    s2dB = 20 * np.log10(np.abs(s2) + 1e-8)
    plt.figure(figsize=(12, 10))


    plt.subplot(2, 1, 1)
    plt.imshow(s1dB, aspect='auto', origin='lower', cmap='viridis')
    plt.title("Original Sound Spectrogram (dB)")
    plt.xlabel("Time")
    ...
    # Target Spectrogram
    plt.subplot(2, 1, 2)
    plt.imshow(s2dB, aspect='auto', origin='lower', cmap='viridis')
    plt.title("Post RIR Spectrogram (dB)")
    plt.xlabel("Time")
    plt.ylabel("Frequency")
    plt.colorbar(label="Amplitude (dB)")

    # Save the spectrogram figure
    spectrogram_plot_path = os.path.join(os.getcwd(), f'combined_spectrograms_dB_RoomGeneration_voice_{voice_idx}.png')
    plt.tight_layout()
...
class data_generator:
    ...
    def generateRoom(self):
        ...
        self.num_of_speakers = 2#random.randint(4,7)
    ...
    def Generate_bg_signals(self, mic_array_height, fs, total_seconds=3):
        ...
        # Generate background source location
        bg_radius = np.random.uniform(low=10.0, high=20.0)
        bg_theta = np.random.uniform(low=0, high=2 * np.pi)
        bg_loc = [
            bg_radius * np.cos(bg_theta) + roomDims[0]/2,
            bg_radius * np.sin(bg_theta) + roomDims[1]/2,
        ...
        for soundfile in soundfiles:
            signal, fs_read = librosa.load(soundfile, sr=fs)
        ...
        signal_total = np.array(signal_total)
        write("sumOfBG.wav", fs, signal_total.sum(axis=0))
        signal_total = signal_total.sum(axis=0)
        # Add each speaker as a source in the room at the same location
       
        
        room.add_source(bg_loc, signal=signal_total)
        ...
        # Sum the contributions from all background sources for each microphone
        bg_signals = np.sum(premix, axis=1)  # Sum over sources, shape: (num_mics, num_samples)
        bg_signals = room.mic_array.signals[:,:total_samples]
        bg_loc = [bg_loc[0] - roomDims[0]/2,bg_loc[1] - roomDims[1]/2,bg_loc[2]]
        return bg_loc, bg_signals, speakers

    def Generate_bg_signals_(self, mic_array_height, fs, total_seconds=3):
        # Set up the room with the specified absorption and maximum order
        roomDims = [round(random.uniform(40, 50), 4)
                        ,round(random.uniform(40, 50), 4)
                        ,round(random.uniform(3, 4), 4)]
        ...
        signal_total =np.array(signal_total)
        signal_total = signal_total.sum(axis = 0)
        write("sumOfBG.wav",  44100,signal_total)
        ...
        bg_loc = [bg_loc[0] - roomDims[0]/2,bg_loc[1] - roomDims[1]/2,bg_loc[2]]
        return bg_loc, premix, speakers
    
    def Generate_bg_signals_corners(self, mic_array_height, fs, total_seconds=3):
        # Set up the room with the specified absorption and maximum order
    ...
    def generate_channels_V2(self,rt60tgt = 0.7,snr = 8,sir = 1,duration = 3,bg = False):  
        try:
            ...
            for voice_idx, s in self.speaker_soundfile_map.items():
                ...
                e_absorption = random.uniform(0.1, 0.99)
                ...
                while not check_distance:
                    ...
                    if all(euclidean(voice_loc,pos) >= 0.4 for pos in voice_positions):
                        voice_positions.append(voice_loc)
                        check_distance = True
                ...
                save_time_and_freq_plots(fg_signals[0],audio,voice_idx)
            ...
            if True:
                bg_loc, bg_signals,bg_speakers = self.Generate_bg_signals(mic_array_height,fs ,total_seconds = 3)
                ...
                #write("outputTest_bg_ch0.wav",  44100,bg_signals[0])
                #write("outputTest_bg_ch1.wav",  44100,bg_signals[1])
                #write("outputTest_bg_ch6.wav",  44100,bg_signals[5])
