...
config = dict(
    ...
    num_workers = 20,
    dataset="VCTK",
    architecture="Cos_0610_simple")
print(config)
def model_pipeline(hyperparameters):

    # tell wandb to get started
    with wandb.init(project="Cos_0910_simple", config=hyperparameters):
...
def make(config):
    ...
    if True:
        ...
        checkpoint_path = os.path.join(checkpoints_dir, "{}_Cos_simple_0910.pt".format(int(start_epoch) - 1))
...
def get_data(train=False, val=False, test=False):
    ...
    train_dir = Path("../gannot-lab-UG/ilya_tomer/OUTPUTS/OUTPUTS_BASIC_TRAIN/Train_simple_3d_0910_")
    val_dir = Path("../gannot-lab-UG/ilya_tomer/OUTPUTS/OUTPUTS_BASIC_TRAIN/Val_simple_3d_0910")
    test_dir = Path("../gannot-lab-UG/ilya_tomer/OUTPUTS/OUTPUTS_BASIC_TRAIN/Test_simple_3d_0910")
...
def train(model, loader,val_loader, criterion, optimizer, config):
    ...
    for epoch in tqdm(range(config.epochs)):
        ...
        if epoch % 1 == 0:
                torch.save(
                    model.state_dict(),
                    os.path.join(yaml_checkpoints_dir,name,"{}_Cos_simple_0910.pt".format(epoch+start_epoch))
                )
