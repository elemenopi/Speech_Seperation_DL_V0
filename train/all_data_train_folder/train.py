...
def validate_epoch(model: nn.Module, device: torch.device, val_loader: torch.utils.data.dataloader.DataLoader, log_interval: int = 20) -> float:
    model.eval()
    val_loss = 0
    with torch.no_grad():
        for batch_idx, (data, label_voice_signals, window_idx) in enumerate(val_loader):
            data = data.to(device)
            label_voice_signals = label_voice_signals.to(device)
            window_idx = window_idx.to(device)

            data,mean,stds = normalize_input(data)
            ...
            output_signal = unnormalize_input(output_signal)
            output_voices = output_signal[:,0]
...
def train(train_dir, test_dir, n_mics, sr, mic_radius,n_workers,pretrain_path,start_epoch):
    data_train = customdataset(train_dir)
    data_test = customdataset(test_dir)
    print(len(data_train))
    use_cuda = torch.cuda.is_available()
    device = torch.device('cuda' if use_cuda else 'cpu')
    num_workers = min(multiprocessing.cpu_count(), n_workers)

    kwargs = {
        'num_workers': num_workers,
        'pin_memory': True
        } if use_cuda else {}
    train_loader = DataLoader(data_train,batch_size=32,shuffle=True,**kwargs)
    test_loader = DataLoader(data_test,batch_size=32,**kwargs)
    model = CosNetwork()
    model.to(device)
    if pretrain_path:
...
if __name__ == "__main__":
    ...
    data_train = customdataset(train_dir)
    print(f"Training dataset loaded: {data_train}")
    print("length")
    print(len(data_train))
    print("###")
    ...
    #model = CosNetwork()
    #model.to(device)
    train_loader = DataLoader(data_train,batch_size = batch_size, shuffle = True, **kwargs )
    ...
    if start_epoch:
        ...
        checkpoint_path = os.path.join(checkpoints_dir, "{}_4layer.pt".format(int(start_epoch) - 1))
    ...
    try:
        for epoch in range(start_epoch,epochs+1):
            ...
            torch.save(
                model.state_dict(),
                os.path.join(checkpoints_dir,name,"{}_4layer.pt".format(epoch))
            )
            print(f"train_loss: {train_losses}")
    ...
    #sys.path.insert(0,'/home/dsi/lipovai/clonedProject/RIRnewv/train/demucs.py')
